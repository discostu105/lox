...
- `lox daemon [--poll]` — WebSocket/polling automation daemon
- `lox automation list/edit/check` — automation rule management
- `lox service install/status/logs/uninstall` — systemd service management
...
- Local timezone support for automation time windows (`timezone:` config field)
...
- Automation time windows now use local time instead of UTC
